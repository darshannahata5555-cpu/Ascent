...
function Mirage({ onNav }) {
  ...
  // masonry-ish grid with varied heights
    ...
    <PageBackground>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '40px 28px 80px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            ...
            <h1 className="display" style={{ fontSize: 72, marginTop: 14, letterSpacing: '-0.03em' }}>The weekend, as seen.</h1>
          </div>
        ...
        <div style={{ marginTop: 32, columns: 4, columnGap: 14 }}>
          {filtered.map((p, i) => (
            ...
            >
              <ImgPlaceholder w="100%" h={p.h} tone={p.tone} badge={p.c} />
              <div style={{
                position: 'absolute', inset: 0,
                background: 'linear-gradient(180deg, transparent 60%, rgba(0,0,0,0.6))',
                opacity: 0, transition: 'opacity .3s', display: 'flex', alignItems: 'flex-end', padding: 14,
              }}
                onMouseEnter={e => e.currentTarget.style.opacity = 1}
                onMouseLeave={e => e.currentTarget.style.opacity = 0}
              >
                <div style={{ fontSize: 12, color: 'var(--ink)' }}>{p.caption}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{
          ...
          display: 'grid', placeItems: 'center', zIndex: 100, padding: 40,
          backdropFilter: 'blur(20px)',
        }}>
          <div style={{ maxWidth: 1000, width: '100%', position: 'relative' }}>
            <ImgPlaceholder w="100%" h={600} tone={lightbox.tone} badge={lightbox.c} />
            <div style={{ marginTop: 14, display: 'flex', justifyContent: 'space-between' }}>
              <div style={{ color: 'var(--ink)', fontSize: 14 }}>{lightbox.caption}</div>
              <div style={{ fontFamily: 'var(--f-mono)', fontSize: 11, color: 'var(--ink-mute)' }}>ESC · CLOSE</div>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <PageBackground>
      ...
      <div style={{ maxWidth: 880, margin: '0 auto', padding: '40px 28px 80px' }}>
        ...
        <h1 className="display" style={{ fontSize: 64, marginTop: 14, letterSpacing: '-0.02em' }}>Notice board.</h1>
            ...
            <div key={i} className="glass" style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {p.pin && <span className="chip violet" style={{ padding: '2px 8px', fontSize: 9 }}>📌 PINNED</span>}
                  <span style={{ fontFamily: 'var(--f-display)', fontSize: 18, fontWeight: 600, letterSpacing: '-0.01em' }}>{p.t}</span>
                </div>
                <span style={{ fontFamily: 'var(--f-mono)', fontSize: 11, color: 'var(--ink-mute)' }}>{p.when}</span>
              </div>
  ...
  return (
    <PageBackground>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 28px 80px' }}>
        ...
        <h1 className="display" style={{ fontSize: 56, marginTop: 14, letterSpacing: '-0.02em' }}>Something missing?</h1>
        ...
        <div style={{ marginTop: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flex: 1, maxWidth: 520, minWidth: 280 }}>
            <input className="input" placeholder="Search by keyword…" />
            <button className="btn primary sm">+ Post item</button>
          </div>
        </div>
        ...
        <div style={{ marginTop: 28, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
          {items[tab].map((it, i) => (
            <div key={i} className="glass" style={{ padding: 0, overflow: 'hidden' }}>
              <ImgPlaceholder w="100%" h={160} tone={it.tone} label={`PHOTO ${i+1}`} />
              <div style={{ padding: 16 }}>
                <div style={{ fontFamily: 'var(--f-display)', fontSize: 16, fontWeight: 600 }}>{it.t}</div>
                <div style={{ marginTop: 6, fontSize: 12, color: 'var(--ink-dim)' }}>Last seen · {it.w}</div>
                <div style={{ marginTop: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontFamily: 'var(--f-mono)', fontSize: 10.5, color: 'var(--ink-mute)', letterSpacing: '0.1em' }}>{it.when.toUpperCase()} · {it.by.toUpperCase()}</span>
