...
function Login({ onNav }) {
  ...
  return (
    <PageBackground>
      {/* Back link instead of full navbar */}
      <div style={{ padding: '22px 28px', display: 'flex', justifyContent: 'space-between' }}>
        <div onClick={() => onNav('landing')} style={{ cursor: 'pointer' }}>
          <AscentLogo size={18} />
        </div>
        <a onClick={() => onNav('credentials')} style={{ color: 'var(--ink-dim)', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--f-mono)', letterSpacing: '0.1em' }}>
          NEED CREDENTIALS? →
        </a>
      </div>
      <div style={{ display: 'grid', placeItems: 'center', padding: '60px 24px 120px', minHeight: 620 }}>
        <div className="fade-up glass-strong" style={{
          width: 440, padding: 36, position: 'relative', overflow: 'hidden',
        }}>
          {/* Animated border glow */}
          <div style={{
            position: 'absolute', inset: -1, borderRadius: 'inherit',
            background: 'conic-gradient(from 0deg, transparent 0%, rgba(127,119,221,0.45) 20%, transparent 40%, transparent 100%)',
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{
              ...
              <div style={{ width: 8, height: 8, borderRadius: '50%', marginTop: 5, background: 'var(--teal)', boxShadow: '0 0 8px var(--teal)' }}/>
  ...
  if (submitted) {
    return (
      <PageBackground>
        ...
        <div style={{ display: 'grid', placeItems: 'center', padding: '60px 24px', minHeight: 620 }}>
          <div className="fade-up glass-strong" style={{ width: 520, padding: 48, textAlign: 'center' }}>
            <div style={{
  ...
  return (
    <PageBackground>
      <div style={{ padding: '22px 28px', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <a onClick={() => onNav('login')} style={{ color: 'var(--ink-dim)', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--f-mono)', letterSpacing: '0.1em' }}>
          ALREADY HAVE ACCESS? →
        </a>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px 120px' }}>
        ...
        <h1 className="display" style={{ fontSize: 56, marginTop: 16, letterSpacing: '-0.02em', maxWidth: 700 }}>
          Tell us who you are.
        </h1>
        ...
        <div className="glass-strong fade-up" style={{ marginTop: 48, padding: 40 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 14, height: 14, border: '1px solid var(--glass-border-strong)', borderRadius: 3, display: 'grid', placeItems: 'center' }}>
            ...
            <button onClick={() => setSubmitted(true)} className="btn primary" style={{ padding: '14px 26px' }}>
              Request credentials →
            </button>
          </div>
        </div>
